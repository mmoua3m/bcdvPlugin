...
videojs.registerPlugin('localizationPlugin', function() {
  ...
  function dvButton(){
    // Add CC button back here if possible
    const dvButton = myPlayer.getChild('ControlBar').addChild('button', {
      controlText: myPlayer.localize("Described Video"),
      ...
      clickHandler: function(event) { 
        ...
        if ( watchingOriginalVideo == true ) { //If we have the original video loaded and we click the button, load the described video
          ...
          dvButton.controlText(myPlayer.localize("Original Video"))  //Change our button text
          ...
          dvButton.controlText(myPlayer.localize("Described Video")) //Change our button text
  ...
  }, 600) //500 is the time to wait, seems like the perfect amount of time to let all data load.
